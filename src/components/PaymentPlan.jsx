      ...
      <div className="max-w-3xl">
        ...
        <div className="space-y-0">
          {payments.map((p, idx) => {
            ...
            return (
              <div key={idx} data-pdf-block={`payment-${idx}`} className="group relative flex gap-8 pb-10 last:pb-0">
                {/* Linea verticale */}
                    ...
                    <>
                      <div className="flex flex-col md:flex-row md:items-baseline justify-between gap-2 mb-2">
                        <div className="flex items-center gap-2.5">
                          ...
                          <span className="bg-[#fef3c7] text-[#92400e] text-[8px] font-black uppercase tracking-[0.1em] px-1.5 py-[2px] rounded">
                            Fornitura
                          </span>
                        </div>
                      ...
                      {p.materialItems && p.materialItems.length > 0 && (
                        <div className="mt-4 pt-3 border-t border-gray-100 space-y-1.5">
                          <p className="text-[9px] font-black uppercase tracking-[0.15em] text-[#a1a1a6] mb-2">
                            Dettaglio fornitura
                          </p>
                          {p.materialItems.map((mi, miIdx) => (
                            <div key={miIdx} className="flex items-center justify-between text-[12px]">
                              <div className="flex items-center gap-2 min-w-0 flex-1">
                                <span className="text-[#c7c7cc] text-[10px]">•</span>
                                <span className="text-[#636366] font-medium truncate">{mi.description}</span>
                                {mi.quantity && mi.unit && (
                                  <span className="text-[#a1a1a6] text-[10px] shrink-0">
                                    ({mi.quantity} {mi.unit})
                                  </span>
                                )}
                              </div>
                      ...
                      {p.description && (
                        <p className="mt-3 text-[13px] leading-relaxed text-[#86868b] max-w-xl italic">
                          {p.description}
                        </p>
                      )}
                    </>
